    ...
    T: NodeValue,
{
    ...
    fn update(
        ...
        let ret = self.root.update_internal::<H, Arity>(self.height, pos, &traversal_path, elem)?;
    ...
    fn non_membership_verify(
        ...
        if *pos != proof.pos {
            return Err(PrimitivesError::ParameterError("Inconsistent proof index".to_string()));
        }
    ...
    T: NodeValue,
{
    ...
    fn non_membership_remember(
        ...
        if matches!(&proof.proof[0], MerkleNode::Empty) {
            ...
            traversal_path.iter().zip(proof.proof.iter().skip(1)).try_fold(
                empty_value,
...
mod mt_tests {
    ...
    fn test_non_membership_lookup_and_verify_helper<F: RescueParameter>() {
        ...
        let mut proof = mt.universal_lookup(BigUint::from(3u64)).expect_not_found().unwrap();
    ...
    fn test_universal_mt_forget_remember_helper<F: RescueParameter>() {
        let mut mt = RescueSparseMerkleTree::<BigUint, F>::from_kv_set(
            10,
            [(BigUint::from(0u64), F::from(1u64)), (BigUint::from(2u64), F::from(3u64))],
        )
        .unwrap();
        let root = mt.commitment().digest();

        // Look up and forget an element that is in the tree.
        let (lookup_elem, lookup_mem_proof) =
            mt.universal_lookup(BigUint::from(0u64)).expect_ok().unwrap();
        ...
        assert!(matches!(mt.universal_forget(0u64.into()), LookupResult::NotInMemory));
        assert!(matches!(mt.universal_lookup(BigUint::from(0u64)), LookupResult::NotInMemory));

        // We should still be able to look up an element that is not forgotten.
        let (elem, proof) = mt.universal_lookup(BigUint::from(2u64)).expect_ok().unwrap();
        assert_eq!(elem, &3u64.into());
        assert!(RescueSparseMerkleTree::<BigUint, F>::verify(&root, BigUint::from(2u64), &proof)
            .unwrap()
            .is_ok());
        ...
        assert!(mt.non_membership_verify(BigUint::from(1u64), &lookup_non_mem_proof).unwrap());
        assert!(mt.non_membership_verify(BigUint::from(1u64), &non_mem_proof).unwrap());
        ...
        match mt.universal_lookup(BigUint::from(1u64)) {
            LookupResult::NotFound(proof) => {
                assert!(mt.non_membership_verify(BigUint::from(1u64), &proof).unwrap());
            },
            res => {
        ...
        mt.universal_forget(BigUint::from(2u64)).expect_ok().unwrap();
        assert!(matches!(mt.universal_lookup(BigUint::from(0u64)), LookupResult::NotInMemory));
        assert!(matches!(mt.universal_lookup(BigUint::from(1u64)), LookupResult::NotInMemory));
        assert!(matches!(mt.universal_lookup(BigUint::from(2u64)), LookupResult::NotInMemory));
        ...
        mt.remember(0u64.into(), F::from(1u64), &bad_mem_proof).unwrap_err();
        ...
        mt.non_membership_remember(1u64.into(), &bad_non_mem_proof).unwrap_err();
        ...
        mt.remember(BigUint::from(1u64), F::from(0u64), &forge_mem_proof).unwrap_err();

        let forge_non_mem_proof = non_mem_proof.clone();
        assert!(matches!(forge_non_mem_proof.proof[0], MerkleNode::Empty));
        mt.non_membership_remember(0u64.into(), &forge_non_mem_proof).unwrap_err();

        // Remember an occupied and an empty  sub-tree.
        mt.remember(0u64.into(), F::from(1u64), &mem_proof).unwrap();
        mt.non_membership_remember(1u64.into(), &non_mem_proof).unwrap();

        // We should be able to look up everything we remembered.
        let (elem, proof) = mt.universal_lookup(BigUint::from(0u64)).expect_ok().unwrap();
        assert_eq!(elem, &1u64.into());
        assert!(RescueSparseMerkleTree::<BigUint, F>::verify(&root, BigUint::from(0u64), &proof)
            .unwrap()
            .is_ok());
